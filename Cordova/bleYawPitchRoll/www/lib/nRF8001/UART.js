...
angular.module('nRF8001', ['ionic'])
  ...
  .factory('nRF8001.Device', ['$q', function($q) {
    var Device = function (ble, device, scope) {
      this._device = device;
...
//            self.close(self.scannedDevices[i]);
      ...
      /**
       * 
       * @param intTimeoutMax How many seconds until stopScan() is called? Set to zero for no timeout.
       * @returns null
       */
      self.startScan = function(intTimeoutMax) {
        ...
        self.ble.startScan(
          function(device) {
            console.log("\n\n\n\n\n" + device.address + ' = ' + (typeof self.scannedDevices[device.address] === 'object')  + "\n\n\n\n");
            
            
            if (typeof self.scannedDevices[device.address] !== 'object' ) {
              var newDevice = new Device(self.ble, device, self.scope);

              if (self.readCallback) {
                newDevice.readCallback = self.readCallback;
              }

              self.scannedDevices[device.address] = newDevice;

              console.log('Adding device');
              console.log(self.scannedDevices);
      ...
      // Stops scanning for bluetooth
      self.stopScan = function (skipScopeApply) {
        console.log('Stop scanning!');
        self.isScanning = false;
        self.ble.stopScan();
        
        if (self.stopScanCallback) {
          self.stopScanCallback();
        }
      ...
      //This resets bluetooth after calling stopScan() to avoid weird Bluetooth connection bugs.
      // Call before ending your app.
      self.resetBluetooth = function () {
        self.ble.reset();
        self.scope.$apply();
      };
    }]);